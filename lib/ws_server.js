...
export async function createWsServer(config, logger, engine, app) {
        ...
        if (message.channel === 'log') {
            ...
            jaysonWsServer.call(body, {}, function (error, success) {
                forSubscriptions(pgClient, 'logs', function (row) {
                    ...
                    const topics = row.filter?.topics?.reduce((accumulator, value) => accumulator.concat(value), []) || [];
    ...
    const blockHeight = (await engine.getBlockHeight()).unwrap();
    setTimeout(notifyNewHeads, newHeadsInterval, pgClient, expressWsApp, jaysonWsServer, blockHeight);
    return true;
}
...
async function notifyNewHeads(pgClient, expressWsApp, jaysonWsServer, blockNumber) {
    const result = await pgClient.query('SELECT 1 FROM block WHERE id = $1 LIMIT 1', [
        blockNumber,
    ]);
    ...
    setTimeout(notifyNewHeads, newHeadsInterval, pgClient, expressWsApp, jaysonWsServer, blockNumber);
}
