    ...
    expressWsApp.app.ws('/', function (ws, req) {
        ...
        ws.on('message', function (msg) {
            try {
                ...
                jaysonWsServer.call(parsedObject, {
    ...
    pgClient.on('notification', (message) => {
        ...
        if (message.channel === 'block') {
            ...
            pgClient
                ...
                .then(function (res) {
                pgClient
                    .query("SELECT COALESCE(array_agg(sec_websocket_key), '{}') AS wskeys FROM subscription WHERE type = 'newHeads'")
        ...
        if (message.channel === 'transaction') {
            ...
            pgClient
                .query('SELECT * FROM eth_getTransactionByBlockNumberAndIndex($1, $2) LIMIT 1', [parsedObject.blockId, parsedObject.index])
                .then(function (res) {
                pgClient
                    .query("SELECT COALESCE(array_agg(sec_websocket_key), '{}') AS wskeys FROM subscription WHERE type = 'newPendingTransactions'")
                    .then(function (sec_websocket_keys) {
                    expressWsApp.getWss().clients.forEach(function (client) {
                        if (sec_websocket_keys.rows[0].wskeys.indexOf(client.id) > -1) {
                            client.send(JSON.stringify(exportJSON(res.rows[0])));
