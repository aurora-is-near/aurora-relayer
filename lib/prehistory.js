...
export class PrehistoryIndexer {
    ...
    async start(blockID, mode) {
        ...
        for (let offset = blockID; offset < this.network.firstBlock; offset += step) {
            const query = sql
                .select()
                .from('blocks')
                .where(sql(`block_height >= ${offset} AND block_height < ${offset + step}`))
                .limit(step);
            ...
            for (let i = 0; i < step; i++) {
                ...
                const parentHash = computeBlockHash(blockID_ - 1, contractID.toString(), this.network.chainID);
