...
export async function routeRPC(provider, method, params) {
    ...
    switch (method) {
        ...
        case 'web3_clientVersion': {
            if (params.length > 0)
                throw new TooManyArguments(0);
            return 'Aurora-Relayer/0.0.0'; // TODO
        }
        case 'web3_sha3': {
            if (params.length < 1)
                throw new MissingArgument(0);
            if (params.length > 1)
                throw new TooManyArguments(1);
        ...
        case 'eth_getTransactionCount': {
            if (params.length < 1)
                throw new MissingArgument(0, "cannot request transaction count without specifying address");
            if (params.length > 2)
                throw new InvalidArguments();
