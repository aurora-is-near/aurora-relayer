...
export class Indexer {
    ...
    async indexBlock(blockID) {
        for (;;) {
            ...
            const transactions = await block.transactions.map(async (transaction, transactionIndex) => {
                return await this.indexTransaction(blockID, transactionIndex, transaction);
            });
            const blockData = {
    ...
    async indexTransaction(blockID, transactionIndex, transaction) {
        ...
        const logs = await (transaction.result?.result?.logs || []).map(async (event, eventIndex) => {
            return await this.indexEvent(blockID, transactionIndex, eventIndex, event);
        });
    ...
    async indexEvent(blockID, transactionIndex, eventIndex, event) {
