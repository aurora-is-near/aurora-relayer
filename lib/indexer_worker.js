...
export class Indexer {
    ...
    async indexTransaction(blockID, transactionIndex, transaction) {
        ...
        try {
            ...
            await this.pgClient.query(`NOTIFY transaction, ${format.literal(transaction.hash)}`);
        }
        catch (error) {
    ...
    async notifyNewHeads(blockID) {
        if (this.pendingHeadBlock == 0) {
            this.pendingHeadBlock = blockID;
        }
        for (;;) {
            if (await this.isBlockIndexed(this.pendingHeadBlock)) {
                await this.pgClient.query(`NOTIFY block, ${format.literal(this.pendingHeadBlock.toString())}`);
    ...
    async isBlockIndexed(blockID) {
        const result = await this.pgClient.query(`SELECT 1 FROM block WHERE id = $1 LIMIT(1)`, [this.pendingHeadBlock]);
