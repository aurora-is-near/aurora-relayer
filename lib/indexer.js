...
export class Indexer {
    ...
    async indexBlock(blockID) {
        ...
        let timestamp = Math.floor(Date.now() / 1000); // initializtion
        for (;;) {
            ...
            if (proxy.isErr()) {
                ...
                if (error.startsWith('[-32000] Server error: DB Not Found Error')) {
                    if (blockID < currentBlockHeight) {
                        ...
                        const emptyBlock = generateEmptyBlock(blockID, contractID.toString(), this.network.chainID, timestamp + 1 // arbitrary timestamp > previous block.timestamp + 1
                        );
            ...
            timestamp = block.timestamp; // it is used only for empty blocks
