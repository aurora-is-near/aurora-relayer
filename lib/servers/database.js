...
export class DatabaseServer extends SkeletonServer {
    ...
    async eth_gasPrice(_request) {
        ...
        return intToHex(BigInt((allowedQuota == -1 ? this.config.defaultGasPrice : allowedQuota) || 0));
    }
    ...
    async eth_sendRawTransaction(request, transaction) {
        ...
        return (await this.engine.submit(transactionBytes)).match({
            ok: (wrappedSubmitResult) => {
                ...
                this.profiles.storeTransaction(request.token(), bytesToHex(transactionHash), gasPrice);
