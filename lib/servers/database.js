...
import { InvalidArguments, RevertError, TransactionError, UnexpectedError, UnknownFilter, UnsupportedMethod, } from '../errors.js';
...
export class DatabaseServer extends SkeletonServer {
    ...
    async eth_getLogs(_request, filter) {
        ...
        const query = sql
            .select('b.id AS "blockNumber"', 'b.hash AS "blockHash"', 't.index AS "transactionIndex"', 't.hash AS "transactionHash"', 'e.index AS "logIndex"', 't.to AS "address"', "string_to_array(concat('0x',encode(e.topics[1], 'hex'), ',', '0x', encode(e.topics[2], 'hex'), ',', '0x', encode(e.topics[3], 'hex'), ',', '0x', encode(e.topics[4], 'hex')), ',') AS \"topics\"", 'coalesce(e.data, repeat(\'\\000\', 32)::bytea) AS "data"', '0::boolean AS "removed"')
            .from('event e')
    ...
    async eth_getTransactionReceiptsByBlockNumber(_request, blockNumber) {
        const blockNumber_ = parseBlockSpec(blockNumber) != 0
            ? parseBlockSpec(blockNumber) || (await this._fetchCurrentBlockID())
            : parseBlockSpec(blockNumber);
        try {
            const { rows, } = await this._query('SELECT * FROM eth_getTransactionReceiptsByBlockNumber($1)', [blockNumber_]);
    ...
    async eth_getTransactionsByBlockNumber(_request, blockNumber) {
        const blockNumber_ = parseBlockSpec(blockNumber) != 0
            ? parseBlockSpec(blockNumber) || (await this._fetchCurrentBlockID())
            : parseBlockSpec(blockNumber);
        try {
            const { rows, } = await this._query('SELECT * FROM eth_getTransactionsByBlockNumber($1)', [blockNumber_]);
