...
export class DatabaseServer extends SkeletonServer {
    ...
    async eth_getLogs(filter) {
        ...
        const query = sql.select('b.id AS "blockNumber"', 'b.hash AS "blockHash"', 't.index AS "transactionIndex"', 't.hash AS "transactionHash"', 'e.index AS "logIndex"', 't.to AS "address"', 'e.topics AS "topics"', 'e.data AS "data"', '0::boolean AS "removed"')
            .from('event e')
    ...
    async eth_getTransactionReceipt(transactionHash) {
        ...
        try {
            ...
            return (!rows || !rows.length) ? null : exportJSON(rows.map((row) => {
                row['logs'] = []; // TODO: fetch
                return row;
            })[0]);
        }
    ...
    async eth_newFilter(filter) {
        ...
        const { rows: [{ id }] } = await this._query('SELECT eth_newFilter($1::inet, $2::blockno, $3::blockno, $4::address[], $5::jsonb) AS id', ['0.0.0.0', fromBlock, toBlock, addresses, topics ? JSON.stringify(topics) : null]); // TODO: IPv4
        return intToHex(id);
    }
