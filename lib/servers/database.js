...
export class DatabaseServer extends SkeletonServer {
    ...
    async eth_getLogs(_request, filter) {
        ...
        const query = sql
            .select('b.id AS "blockNumber"', 'b.hash AS "blockHash"', 't.index AS "transactionIndex"', 't.hash AS "transactionHash"', 'e.index AS "logIndex"', 'coalesce(e.from, \'\\x0000000000000000000000000000000000000000\')::address AS "address"', "string_to_array(concat('0x',encode(e.topics[1], 'hex'), ',', '0x', encode(e.topics[2], 'hex'), ',', '0x', encode(e.topics[3], 'hex'), ',', '0x', encode(e.topics[4], 'hex')), ',') AS \"topics\"", 'coalesce(e.data, repeat(\'\\000\', 32)::bytea) AS "data"', '0::boolean AS "removed"')
            .from('event e')
    ...
    async _fetchEvents(transactionID) {
        const { rows } = await this._query(`SELECT
          ...
          e.index AS "logIndex",
          coalesce(e.from, \'\\x0000000000000000000000000000000000000000\')::address AS "address",
