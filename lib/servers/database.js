...
import { InvalidAddress, InvalidArguments, RevertError, TransactionError, UnexpectedError, UnknownFilter, UnsupportedMethod, GasPriceTooLow, LimitLogsExceeded, } from '../errors.js';
...
export class DatabaseServer extends SkeletonServer {
    ...
    async eth_call(_request, transaction, blockNumberOrHash) {
        ...
        try {
            return (await this.engine.view(from, to, value, data, {
                ...
                ok: (result) => {
                    if (!Buffer.isBuffer(result) &&
                        result.isErr()) {
                        const errorInstance = result.err().unwrap()
                            .constructor.name;
            ...
            if (error?.message?.includes('FunctionCallError(WasmTrap(Unreachable))')) {
    ...
    async eth_gasPrice(_request) {
        const minGasPrice = this.config.minGasPrice !== undefined ? this.config.minGasPrice : 0;
    ...
    async eth_getBalance(_request, address, blockNumber) {
        ...
        const balance = (await this.engine.getBalance(address_, {
    ...
    async eth_getBlockTransactionCountByHash(_request, blockHash) {
        ...
        const { rows: [{ result }], } = await this._query(query);
    ...
    async eth_getFilterChanges(_request, filterID) {
        ...
        const filterData = await this._query(`SELECT
          type,
          poll_block,
          to_block,
          addresses,
          topics
        FROM filter
        WHERE uuid_send(id) = $1 LIMIT 1`, [filterID_]);
    ...
    async eth_getStorageAt(_request, address, key, blockNumber) {
        ...
        const result = (await this.engine.getStorageAt(address_, key, {
            block: this._parseBlockID(blockNumber),
        })).unwrap();
    ...
    async eth_newBlockFilter(_request) {
        ...
        const { rows: [{ uuid_send: id }], } = await this._query(query);
    ...
    async eth_uninstallFilter(_request, filterID) {
        ...
        const { rows, } = await this._query('DELETE FROM filter WHERE uuid_send(id) = $2 AND created_by = $1 RETURNING id', ['0.0.0.0', filterID_]);
    ...
    async _updatePollBlock(filterID_) {
        const latestBlock = await this._fetchCurrentBlockID();
        await this._query('UPDATE filter SET poll_block = $1 + 1 WHERE uuid_send(id) = $2', [latestBlock, filterID_]);
    }
    async _getFilterChangesBlock(filter) {
        const query = sql
            .select("COALESCE(array_agg( REPLACE(b.hash::VARCHAR, '\\x', '0x' ) ORDER BY b.id ASC), '{}') as hashes")
            .from('block b')
            .where(sql.gt('b.id', filter.poll_block));
        const result = await this._query(query);
        return result.rows[0].hashes;
    }
    async _getFilterChangesEvent(filter, filterID_) {
        const filterOptions = {
    ...
    async _fetchTransactions(blockID, fullObject) {
        ...
        if (fullObject) {
          ...
          ORDER BY t.index ASC`, [blockID]);
            ...
            const { rows, } = await this._query(`SELECT t.hash FROM transaction t WHERE t.${idColumn} = $1 ORDER BY t.index ASC`, [blockID]);
    ...
    async _getLogs(filter) {
        ...
        if (filter.blockHash !== undefined && filter.blockHash !== null) {
            ...
            if (blockRange < 0 ||
                ...
                throw new LimitLogsExceeded(this.config.getLogsEventLimit, this.config.getLogsBlockLimit);
            }
    ...
    _parseBlockID(blockSpec) {
        const auroraGenesisBlockHeight = this.config.network === 'mainnet' ? 37157758 : 47354108;
        switch (blockSpec) {
            case undefined:
                return undefined;
            case null:
...
function _eventQuery() {
    return sql
        .select('e.block AS "blockNumber"', 'e.block_hash AS "blockHash"', 'e.transaction_index AS "transactionIndex"', 'e.transaction_hash AS "transactionHash"', 'e.index AS "logIndex"', 'e.from AS "address"', "array(select REPLACE(t.val::varchar, '\\', '0') from unnest(e.topics) with ordinality as t(val)) AS topics", 'coalesce(e.data, repeat(\'\\000\', 32)::bytea) AS "data"', '0::boolean AS "removed"')
        .from('event e')
