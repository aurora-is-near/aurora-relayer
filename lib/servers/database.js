...
export class DatabaseServer extends SkeletonServer {
    ...
    async eth_getBlockByNumber(_request, blockNumber, fullObject) {
        const blockNumber_ = parseBlockSpec(blockNumber) || (await this._fetchCurrentBlockID());
        try {
    ...
    async eth_getTransactionReceiptsByBlockNumber(_request, blockNumber) {
      ...
      WHERE b.id = $1`, [blockNumber_]);
            //assert(receipt, 'receipt is not null');
            //rows[0].logs = await this._fetchEvents(hexToBytes(receipt.transactionHash.toString()));
