...
export class DatabaseServer extends SkeletonServer {
    ...
    async eth_getLogs(filter) {
        ...
        const query = sql
            .select('b.id AS "blockNumber"', 'b.hash AS "blockHash"', 't.index AS "transactionIndex"', 't.hash AS "transactionHash"', 'e.index AS "logIndex"', 't.to AS "address"', 'e.topics AS "topics"', 'coalesce(e.data, repeat(\'\\000\', 32)::bytea) AS "data"', '0::boolean AS "removed"')
            .from('event e')
